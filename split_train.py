    ...
    for task in tqdm(range(n_tasks)):
        ...
        if task !=0:
            ...
            for i in range(task):
                ...
                solver.model.load_state_dict(torch.load('/stored_models/task_{}_split_solver_weights.pth'.format(task - 1)))
                solver.set_mode_train(False)
                
                gan = GAN_generator(code_size, 784)
                gan.load_state_dict(torch.load('/stored_models/task_{}_split_gan_weights.pth'.format(task - 1)))
                gan.eval()
            ...
            torch.save(curr_solver.model.state_dict(), 
                       '/stored_models/task_{}_split_solver_weights.pth'.format(task))
            ...
            torch.save(curr_generator.state_dict(), 
                       '/stored_models/task_{}_split_gan_weights.pth'.format(task))
        ...
        plt.savefig('/stored_models/1acc_split_plot{}_tasks.png'.format(task))
        plt.show()
